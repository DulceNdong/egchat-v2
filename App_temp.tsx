...
const App: React.FC = () => {
  const buildVersion = (import.meta as any).env?.VITE_BUILD_VERSION || 'local-dev';
  const [currentView, setCurrentView] = useState<string>('home');
  const [previousView, setPreviousView] = useState<string>('home');
  // ── Auth — sesion persistente ─────────────────────────────────
  ...
  const chatRealtimeRef = React.useRef<EventSource | null>(null);
  const realtimeBase = (import.meta as any).env?.VITE_API_URL || '/api';
  ...
  React.useEffect(() => {
    ...
    const streamUrl = `${realtimeBase}/api/chat/stream?token=${encodeURIComponent(token)}`;
    const es = new EventSource(streamUrl);
    chatRealtimeRef.current = es;
    ...
    es.onmessage = (event) => {
      try {
        ...
        if (payload.type === 'new_message' || payload.type === 'chat_updated') {
          ...
          if (currentChatId && payload.chatId && String(payload.chatId) === currentChatId) {
    ...
    es.onerror = () => {
      // EventSource reintenta de forma automática.
    };
  ...
  }, [isAuthenticated, realtimeBase, loadChats, loadMessages, selectedChat?.id]);

  const parseEgchatQR = (payload: string) => {
    const trimmed = payload.trim();
    const match = trimmed.match(/^egchat:\/\/user\/([0-9a-fA-F-]{36})\/(?:(\+?\d+))$/);
    if (match) {
      return { userId: match[1], phone: match[2] };
    }
    const matchPhoneOnly = trimmed.match(/^egchat:\/\/user\/(?:(\+?\d+))$/);
    if (matchPhoneOnly) {
      return { userId: undefined, phone: matchPhoneOnly[1] };
    }
    return null;
  };

  const mapContactsFromApi = (data: any[]) => {
    if (!Array.isArray(data)) return;
    setAllContacts(data.map((c: any) => {
      const user = c?.user || {};
      const contactName = c?.nickname || user?.full_name || c?.name || 'Contacto';
      const contactPhone = user?.phone || c?.phone || '';
      return {
        id: c?.id || user?.id || `${Date.now()}-${Math.random()}`,
        name: contactName,
        phone: contactPhone,
        avatar: (contactName || 'C').split(' ').map((w: string) => w[0]).join('').slice(0, 2).toUpperCase(),
        avatarUrl: user?.avatar_url || c?.avatar_url || '',
        status: (user?.status === 'online' ? 'online' : 'offline') as 'online' | 'offline' | 'away',
        addedDate: c?.created_at ? new Date(c.created_at).toLocaleDateString('es-ES') : new Date().toLocaleDateString('es-ES'),
      };
    }));
  };
  ...
  const refreshContacts = async () => {
    try {
      ...
      mapContactsFromApi(data);
    } catch {}
  };

  const handleAddContactByPhone = async () => {
    const phone = newContactPhone.trim();
    if (!phone) {
      setAddContactError('Ingresa un número de teléfono válido');
      return;
    }

    const normalizedPhone = phone.startsWith('+') ? phone : '+240' + phone.replace(/\D/g, '').slice(-9);
    try {
      setAddContactError('');
      const chat = await chatAPI.createPrivate(undefined, normalizedPhone);
      try {
        const { contactsAPI } = await import('./api');
        await contactsAPI.add(undefined, normalizedPhone, newContactName.trim() || undefined);
      } catch {}
      ...
      if (chat && chat.id) {
        ...
        const otherParticipant = chat.participants?.find((p: any) => p.user_id !== currentUserId.current);
        const contactName = newContactName.trim() || chat.name || otherParticipant?.full_name || otherParticipant?.phone || normalizedPhone;
        const initials = contactName.split(' ').map((w: string) => w[0]).join('').slice(0,2).toUpperCase();
        setSelectedChat({
          id: chat.id,
      ...
      console.error('Error creando chat por teléfono:', error);
      setAddContactError(error?.message || 'No se pudo crear el chat');
    }
  };

  const handleAddContactByQR = async () => {
    if (!qrPayload.trim()) {
      setAddContactError('Pega el contenido del QR');
      return;
    }

    const parsed = parseEgchatQR(qrPayload);
    if (!parsed) {
      setAddContactError('Formato de QR inválido');
      return;
    }

    try {
      setAddContactError('');
      const chat = await chatAPI.createPrivate(parsed.userId, parsed.phone);
      try {
        const { contactsAPI } = await import('./api');
        await contactsAPI.add(parsed.userId, parsed.phone, undefined);
      } catch {}
      await refreshContacts();
      if (chat && chat.id) {
        setShowAddContact(false);
        ...
        setCurrentView('mensajeria');
        const otherParticipant = chat.participants?.find((p: any) => p.user_id !== currentUserId.current);
        const contactName = chat.name || otherParticipant?.full_name || otherParticipant?.phone || parsed.phone || 'Nuevo contacto';
        const initials = contactName.split(' ').map((w: string) => w[0]).join('').slice(0,2).toUpperCase();
        setSelectedChat({
          id: chat.id,
          type: chat.type || 'individual',
      ...
      console.error('Error creando chat desde QR:', error);
      setAddContactError(error?.message || 'No se pudo crear el chat');
    }
  };
  ...
  // Auth check moved to end — after all hooks
  ...
  const [userPIN, setUserPIN] = useState<string>(''); // PIN debe configurarse por el usuario, no debe tener valor por defecto
  ...
  useEffect(() => {
    ...
    authAPI.me().then((u: any) => {
      ...
      // Evitar logout por fallos temporales tras recargas/updates.
      // Solo cerrar sesión si el cliente ya marcó explícitamente "Sesión expirada".
    ...
    import('./api').then(({ contactsAPI }) => {
      contactsAPI.getAll().then((data: any[]) => mapContactsFromApi(data)).catch(() => {});
    });
    // Polling notificaciones cada 10s
    const notifInterval = setInterval(async () => {
      try {
        const { noticiasAPI } = await import('./api');
        // Verificar mensajes nuevos
        const chats = await chatAPI.getChats();
        if (Array.isArray(chats)) setRealChats(chats);
      } catch {}
    }, 10000);
  ...
  const renderNotificationsPanel = () => {
    ...
    const notifs = [
      ...
      {
        id: 3, icon: 'zap', label: 'Actualizaci?n', desc: 'Versi?n 2.5 disponible', time: '1h', color: '#a855f7',
        detail: {
          title: 'Nueva versi?n disponible',
          from: 'EGCHAT Sistema',
          fromAvatar: 'EG',
          message: 'La versi?n 2.5 incluye mejoras de rendimiento, nuevos iconos y correcci?n de errores.',
        ...
        <div onClick={e => e.stopPropagation()} style={{ width: '100%', background: '#FFFFFF', borderRadius: '18px 18px 0 0', border: '1px solid rgba(0,0,0,0.07)', padding: '20px 16px 32px' }}>
          ...
          {addContactTab === 'phone' ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              ...
              {addContactError ? (
                <div style={{ color: '#dc2626', fontSize: '12px', minHeight: '18px' }}>{addContactError}</div>
              ) : null}
              <button onClick={handleAddContactByPhone}
                style={{ background: newContactPhone.trim() ? '#00b4e6' : '#94a3b8', border: 'none', borderRadius: '10px', padding: '12px', color: 'white', fontSize: '13px', fontWeight: '700', cursor: newContactPhone.trim() ? 'pointer' : 'not-allowed', outline: 'none', marginTop: '4px' }}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', padding: '8px 0' }}>
              <textarea value={qrPayload} onChange={e => setQrPayload(e.target.value)}
                placeholder="Pega aquí el contenido del código QR"
                rows={4}
                style={{ width: '100%', resize: 'vertical', background: 'rgba(249,250,251,0.88)', border: '1px solid rgba(0,0,0,0.08)', borderRadius: '12px', padding: '12px', color: '#0d0d0d', fontSize: '13px', outline: 'none', fontFamily: 'inherit' }} />
              {addContactError ? (
                <div style={{ color: '#dc2626', fontSize: '12px', minHeight: '18px' }}>{addContactError}</div>
              ) : null}
              <button onClick={handleAddContactByQR}
                style={{ width: '100%', background: qrPayload.trim() ? '#00b4e6' : '#94a3b8', border: 'none', borderRadius: '10px', padding: '12px', color: 'white', fontSize: '13px', fontWeight: '700', cursor: qrPayload.trim() ? 'pointer' : 'not-allowed', outline: 'none' }}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px', padding: '8px 0' }}>
                ...
                <button style={{ width: '100%', background: 'rgba(0,180,230,0.15)', border: '1px solid rgba(0,180,230,0.3)', borderRadius: '10px', padding: '10px 24px', color: '#00b4e6', fontSize: '12px', fontWeight: '600', cursor: 'pointer', outline: 'none' }}>
                  Activar cámara
                </button>
              </div>
            </div>
  ...
  const renderBottomNavigation = () => {
    // Solo mostrar navegación inferior en la página principal
    if (currentView !== 'home') return null;
    ...
    return (
      <div style={{
        ...
        padding: '0 12px',
        zIndex: 1000,
        boxShadow: '0 -4px 12px rgba(0,180,230,0.4)',
        borderRadius: '20px',
        ...
        {navItems.map((item) => (
          ...
          >
            ...
            <div style={{
              ...
              height: '32px',
              color: '#ffffff',
              filter: currentView === item.id ? 'drop-shadow(0 0 6px rgba(255,255,255,0.9))' : 'none',
              transition: 'all 0.2s ease',
              transform: currentView === item.id ? 'translateY(-2px) scale(1.1)' : 'none',
            }}>
  ...
  const renderServicesView = () => {
    ...
    return (
      <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: '#F7F8FA', overflow: 'hidden' }}>
      ...
      case 'mensajeria':
        ...
        if (selectedChat) {
          ...
          const msgs = chatMessages[chatId] || [
            ...
            { id: '2', from: 'me' as const, text: 'Hola! Todo bien por aquí', time: '10:18', status: 'read' as const },
          ...
          const sendChatMessage = async () => {
            ...
            const newMsg = { id: Date.now().toString(), from: 'me' as const, text, time, status: 'pending' as const };
            ...
            if (chatId.length > 10) {
              try {
                ...
                setChatMessages(prev => ({ ...prev, [chatId]: (prev[chatId]||[]).map((m:any) => m.id===newMsg.id ? {...m, status:'delivered'} : m) }));
                // Refresco inmediato; nuevos mensajes llegarán por stream SSE.
                loadMessages(chatId);
              } catch {
                setTimeout(() => setChatMessages(prev => ({ ...prev, [chatId]: (prev[chatId]||[]).map((m:any) => m.id===newMsg.id ? {...m, status:'delivered'} : m) })), 1000);
              }
            } else {
              const msgId = newMsg.id;
              setTimeout(() => setChatMessages(prev => ({ ...prev, [chatId]: (prev[chatId]||[]).map((m:any) => m.id===msgId ? {...m, status:'delivered'} : m) })), 1000);
              setTimeout(() => setChatMessages(prev => ({ ...prev, [chatId]: (prev[chatId]||[]).map((m:any) => m.id===msgId ? {...m, status:'read'} : m) })), 3000);
            }
          };

          return (
            <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }} onClick={() => { if(showChatMenu) setShowChatMenu(false); }}>
              {/* Header conversación */}
        ...
        return (
          <div style={{
            ...
            <div style={{ marginBottom: '8px', flexShrink: 0 }}>
              {/* Barra de busqueda + boton nuevo chat */}
              ...
              {realChats.filter(Boolean).map((chat: any, idx: number) => {
                if (!chat) return null;
                const other = chat.participants?.find((p: any) => p.user_id !== currentUserId.current);
                const name = chat.name || other?.full_name || other?.users?.full_name || 'Chat';
                const initials = name.split(' ').map((w: string) => w[0]).join('').slice(0,2).toUpperCase();
                const lastMsg = chat.last_message;
                const chatId = chat.id || `chat-${idx}`;
                return (
                  <div key={chatId}
                    onClick={()=>{
                      if (chat.id) loadMessages(chat.id);
                      setSelectedChat({id:chat.id||chatId,type:chat.type||'individual',title:name,subtitle:lastMsg?.text||'Sin mensajes',time:lastMsg?new Date(lastMsg.created_at).toLocaleTimeString('es-ES',{hour:'2-digit',minute:'2-digit'}):'',status:'online',icon:'contactos',color:'#00c8a0',initials});
                    }}
              ...
              <div style={{
                ...
                {expandFavoriteContacts && (
                  <div style={{
                    ...
                    {[
                      { id: 1, name: 'Juan', initials: 'JU', color: '#00b4e6' },
                      { id: 2, name: 'María', initials: 'MA', color: '#00c8a0' },
                      { id: 3, name: 'Carlos', initials: 'CA', color: '#f59e0b' },
                      { id: 4, name: 'Ana', initials: 'AN', color: '#8b5cf6' },
                      { id: 5, name: 'Pedro', initials: 'PE', color: '#ef4444' },
                      { id: 6, name: 'Sofia', initials: 'SO', color: '#06b6d4' }
                    ].map((contact) => (
                      <button
                        key={contact.id}
                        style={{
                      ...
                      >
                        <Avatar name={contact.name} size={56} showStatus={false} />
                        <span style={{ 
                          fontSize: '10px', 
                          fontWeight: '600',
                          textAlign: 'center',
              ...
              <div style={{
                ...
                {expandFavoriteGroups && (
                  <div style={{
                    ...
                    {[
                      { id: 1, name: 'Familia', initials: 'FA', color: '#ec4899' },
                      { id: 2, name: 'Trabajo', initials: 'TR', color: '#14b8a6' },
                      { id: 3, name: 'Amigos', initials: 'AM', color: '#f97316' },
                      { id: 4, name: 'Proyecto', initials: 'PR', color: '#6366f1' },
                      { id: 5, name: 'Negocios', initials: 'NE', color: '#84cc16' }
                    ].map((group) => (
                      <button
                        key={group.id}
                        style={{
                      ...
                      >
                        <Avatar name={group.name} size={56} showStatus={false} />
                        <span style={{ 
                          fontSize: '10px', 
                          fontWeight: '600',
                          textAlign: 'center',
                  ...
                  <div style={{
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                      <div>
                        ...
                        <div style={{ fontSize: '12px', color: '#0d0d0d', fontWeight: '500' }}>2.5.0</div>
                      </div>
                      <div>
  ...
  // ── Auth check — todos los hooks ya declarados ──────────────────
  if (!isAuthenticated) return <AuthScreen onAuth={(user) => {
    if (user) {
      ...
      if (savedAvatar && !user.avatar_url) {
        import('./api').then(({userAPI}: any) => userAPI.updateProfile({avatar_url: savedAvatar}).catch(()=>{}));
      }
    }
    ...
    <div style={{
                ...
                <div style={{ padding:'14px 16px 0' }}>
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'8px', marginBottom:'14px' }}>
                    {[
                      ...
                      <button key={h?.id || `hotel-${i}`} onClick={()=>setSvcData(p=>({...p,hotelId:h?.id || `hotel-${i}`,hotelLabel:h?.label || 'Hotel',hotelPrice:h?.price || ''}))} style={{ background:svcData.hotelId===(h?.id || `hotel-${i}`)?(h?.color || '#00b4e6')+'15':'#fff', border:`1.5px solid ${svcData.hotelId===(h?.id || `hotel-${i}`)?(h?.color || '#00b4e6'):'#F0F2F5'}`, borderRadius:'12px', padding:'12px', cursor:'pointer', outline:'none', textAlign:'left', boxShadow:'0 1px 3px rgba(0,0,0,0.05)' }}>
                        <div style={{ fontSize:'12px', fontWeight:'700', color:'#111827', marginBottom:'3px' }}>{h?.label || 'Hotel'}</div>
                        <div style={{ fontSize:'10px', color:'#9CA3AF', marginBottom:'5px' }}>{h?.sub || ''}</div>
                        <div style={{ fontSize:'11px', fontWeight:'700', color:h?.color || '#00b4e6' }}>{h?.price || ''}</div>
                      </button>
                    ))}
                  </div>
      ...
      {showContactProfile && (
        <ContactProfileModal
          ...
          onSendMoney={(contact) => {
            if (!contact) return;
            setQuickTransferData({ contactId: contact.id?.toString() || '', contactName: contact.title || contact.name || 'Contacto', amount: '', accountId: bankAccounts[0]?.id || '' });
      ...
      {/* Modal Nuevo Chat — buscar por numero y crear chat real */}
      {showNewChatModal && (
        <div style={{position:'fixed',inset:0,zIndex:1300,background:'rgba(0,0,0,0.5)',backdropFilter:'blur(8px)',display:'flex',alignItems:'flex-end'}}
          onClick={()=>setShowNewChatModal(false)}>
          <div onClick={e=>e.stopPropagation()} style={{width:'100%',background:'#fff',borderRadius:'20px 20px 0 0',padding:'20px 16px 32px',boxShadow:'0 -4px 24px rgba(0,0,0,0.15)'}}>
            <div style={{width:'36px',height:'3px',background:'#E5E7EB',borderRadius:'2px',margin:'0 auto 16px'}}/>
            <h3 style={{fontSize:'16px',fontWeight:'800',color:'#111827',margin:'0 0 6px'}}>Nuevo chat</h3>
            <p style={{fontSize:'13px',color:'#6B7280',margin:'0 0 14px'}}>Numero de telefono del contacto</p>
            <input value={newChatPhone} onChange={e=>setNewChatPhone(e.target.value)}
              placeholder="+240 222 XXX XXX" type="tel"
              style={{width:'100%',background:'#F7F8FA',border:'1.5px solid #E5E7EB',borderRadius:'12px',padding:'13px 14px',fontSize:'15px',color:'#111',outline:'none',boxSizing:'border-box',fontFamily:'inherit',marginBottom:'12px'}}
            />
            <button disabled={newChatSearching||!newChatPhone.trim()}
              onClick={async()=>{
                setNewChatSearching(true);
                try{
                  const BASE='https://egchat-api.onrender.com';
                  const token=authAPI.getToken();
                  const sr=await fetch(`${BASE}/api/contacts/search?q=${encodeURIComponent(newChatPhone.trim())}`,{headers:{Authorization:`Bearer ${token}`}});
                  const users=await sr.json();
                  if(!users||users.length===0){alert('Usuario no encontrado en EgChat');return;}
                  const target=users[0];
                  const cr=await fetch(`${BASE}/api/chats/private`,{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify({participant_id:target.id})});
                  const chat=await cr.json();
                  if(chat?.id){
                    await loadChats();
                    setShowNewChatModal(false);setNewChatPhone('');
                    const name=target.full_name||target.phone;
                    const initials=name.split(' ').map((w:string)=>w[0]).join('').slice(0,2).toUpperCase();
                    setSelectedChat({id:chat.id,type:'individual',title:name,subtitle:'Nuevo chat',time:'',status:'online',icon:'contactos',color:'#00c8a0',initials});
                    setCurrentView('mensajeria');
                  }
                }catch(e){alert(e.message||'Error al crear el chat');}
                finally{setNewChatSearching(false);}
              }}
              style={{width:'100%',background:newChatPhone.trim()?'#22c55e':'#E5E7EB',color:newChatPhone.trim()?'#fff':'#9CA3AF',border:'none',borderRadius:'12px',padding:'14px',fontSize:'15px',fontWeight:'700',cursor:newChatPhone.trim()?'pointer':'default'}}
