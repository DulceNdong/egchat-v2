...
const App: React.FC = () => {
  ...
  // ── Roles y permisos ─────────────────────────────────────────────
  const userRole = useRole(isAuthenticated);
  useEffect(() => {
  ...
  // Nuevas vistas: oficial, business, merchant
  ...
  // Fix teclado iOS: solo actualiza CSS variables, sin mover el container
  ...
  React.useEffect(() => {
    ...
    const update = () => {
      ...
      // En IPA nativa esto lo gestiona Capacitor. En PWA, position:fixed queda
      // anclado al layout viewport (detrás del teclado), hay que moverlo con JS.
      ...
      if (isIOS && !isCapacitor) {
        ...
        if (keyboardH > 80) {
          // Teclado visible: subir la barra exactamente encima del teclado
          if (chatBar) chatBar.style.bottom = `${keyboardH}px`;
          if (tabBar) { tabBar.style.visibility = 'hidden'; tabBar.style.pointerEvents = 'none'; }
          // Reducir paddingTop del scroll para que los mensajes no tapen el header
          if (messagesScroll) {
            messagesScroll.style.paddingTop = '8px';
          ...
          if (messagesScroll) messagesScroll.style.paddingTop = '88px';
        }
      }
        ...
        if (selectedChat) {
          ...
          return (
            ...
            <div className="chat-view-container" ref={chatContainerRef} style={{ 
              ...
              {/* Header movido al return principal como position:fixed independiente */}
                  ...
                  return sorted.map((msg) => {
                    ...
                    >
                      ...
                      {(msg as any).type === 'call' ? (() => {
                        ...
                        : msg.text?.startsWith('🎵') ? (() => {
                          const audioUrl = (msg as any).audioUrl || (msg as any).fileUrl || (msg as any).file_url || '';
                          const raw = (msg.text || '').replace(/^🎵 /, '');
                          const match = raw.match(/^(.+?) \((.+?)\)$/);
                          const fileName = match ? match[1] : raw;
                          const fileSize = match ? match[2] : '';
                          const ext = ((msg as any).fileExt || fileName.split('.').pop()?.toLowerCase() || 'mp3').toUpperCase();
                          const isMine = msg.from === 'me';
                          return (
                            <div style={{ minWidth: '240px', maxWidth: '280px' }}>
                              {/* Header con gradiente musical */}
                              <div style={{ background: isMine ? 'linear-gradient(135deg,#7c3aed,#8b5cf6)' : 'linear-gradient(135deg,#6d28d9,#7c3aed)', borderRadius: audioUrl ? '12px 12px 0 0' : '12px', padding: '10px 12px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                                <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'rgba(255,255,255,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg>
                                </div>
                                <div style={{ flex: 1, minWidth: 0 }}>
                                  <div style={{ fontSize: '12px', fontWeight: '700', color: '#fff', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{fileName}</div>
                                  <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.7)', marginTop: '2px' }}>{fileSize}{fileSize ? ' · ' : ''}{ext}</div>
                                </div>
                              </div>
                              {/* Reproductor */}
                              {audioUrl && (() => {
                                const pid = `music-${msg.id}`;
                                return (
                                  <div style={{ background: '#fff', borderRadius: '0 0 12px 12px', padding: '10px 12px', border: '1px solid rgba(139,92,246,0.15)', borderTop: 'none' }}>
                                    <audio id={pid} src={audioUrl} preload="metadata" style={{ display: 'none' }}
                                      onTimeUpdate={() => {
                                        const a = document.getElementById(pid) as HTMLAudioElement;
                                        const bar = document.getElementById(`bar-${pid}`) as HTMLElement;
                                        const te = document.getElementById(`te-${pid}`) as HTMLElement;
                                        if (!a || !bar) return;
                                        bar.style.width = a.duration ? `${(a.currentTime/a.duration)*100}%` : '0%';
                                        if (te) { const r = a.duration - a.currentTime; te.textContent = isNaN(r) ? '0:00' : `${Math.floor(r/60)}:${String(Math.floor(r%60)).padStart(2,'0')}`; }
                                      }}
                                      onLoadedMetadata={() => {
                                        const a = document.getElementById(pid) as HTMLAudioElement;
                                        const te = document.getElementById(`te-${pid}`) as HTMLElement;
                                        if (a && te && !isNaN(a.duration)) te.textContent = `${Math.floor(a.duration/60)}:${String(Math.floor(a.duration%60)).padStart(2,'0')}`;
                                      }}
                                      onEnded={() => {
                                        const a = document.getElementById(pid) as HTMLAudioElement;
                                        const bar = document.getElementById(`bar-${pid}`) as HTMLElement;
                                        const ic = document.getElementById(`ic-${pid}`);
                                        const te = document.getElementById(`te-${pid}`) as HTMLElement;
                                        if (bar) bar.style.width = '0%';
                                        if (ic) ic.innerHTML = '<polygon points="6 3 20 12 6 21 6 3"/>';
                                        if (te && a && !isNaN(a.duration)) te.textContent = `${Math.floor(a.duration/60)}:${String(Math.floor(a.duration%60)).padStart(2,'0')}`;
                                      }}
                                    />
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                      ...
                                      <button onClick={() => {
                                        ...
                                        if (!a) return;
                                        if (a.paused) {
                                          document.querySelectorAll('audio').forEach(x => { if (x !== a) { x.pause(); const oid = x.id; const oic = document.getElementById(`ic-${oid}`); if (oic) oic.innerHTML = '<polygon points="6 3 20 12 6 21 6 3"/>'; } });
                                          a.play().catch(() => {});
                                          if (ic) ic.innerHTML = '<rect x="6" y="4" width="4" height="16" rx="1"/><rect x="14" y="4" width="4" height="16" rx="1"/>';
                                        } else {
                                          a.pause();
                                          if (ic) ic.innerHTML = '<polygon points="6 3 20 12 6 21 6 3"/>';
                                        }
                                      }} style={{ width: '34px', height: '34px', borderRadius: '50%', background: 'linear-gradient(135deg,#7c3aed,#8b5cf6)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                        <svg id={`ic-${pid}`} width="14" height="14" viewBox="0 0 24 24" fill="#fff" stroke="none"><polygon points="6 3 20 12 6 21 6 3"/></svg>
                                      </button>
                                      {/* Barra de progreso */}
                                      <div onClick={e => {
                                        const a = document.getElementById(pid) as HTMLAudioElement;
                                      ...
                                      }} style={{ flex: 1, height: '4px', background: 'rgba(139,92,246,0.15)', borderRadius: '2px', cursor: 'pointer', position: 'relative' }}>
                                        <div id={`bar-${pid}`} style={{ height: '100%', width: '0%', background: 'linear-gradient(90deg,#7c3aed,#8b5cf6)', borderRadius: '2px', transition: 'width 0.1s linear' }}/>
                                      </div>
                                      {/* Tiempo */}
                                      <span id={`te-${pid}`} style={{ fontSize: '10px', color: '#6b7280', fontWeight: '600', minWidth: '30px', textAlign: 'right' }}>0:00</span>
                                      {/* Descargar */}
                                      <button onClick={() => { const a = document.createElement('a'); a.href = audioUrl; a.download = fileName; a.click(); }}
                                        style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#8b5cf6', padding: '2px', display: 'flex', alignItems: 'center' }}>
                                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"><path d="M12 2v13M7 10l5 5 5-5"/><path d="M3 19h18"/></svg>
                                      </button>
                                    </div>
                                  </div>
                                );
  ...
  return (
    ...
    >
      ...
      {/* ── Vistas nuevas: Oficial, Business, Merchant ── */}
      {showOfficialView && (
      ...
      {showBusinessView && (
        <Suspense fallback={null}>
          <BusinessDashboard
            userProfile={userProfile}
            isAuthenticated={isAuthenticated}
            onBack={() => setShowBusinessView(false)}
            onNavigate={(view) => { setShowBusinessView(false); if (view === 'official') setShowOfficialView(true); else if (view === 'merchant') setShowMerchantView(true); else setCurrentView(view); }}
            viewPadding={viewPadding}
          />
        </Suspense>
      )}
      ...
      {showDirectorView && (
        <Suspense fallback={null}>
          <ProviderDirectorDashboard
            userProfile={userProfile}
            isAuthenticated={isAuthenticated}
            onBack={() => setShowDirectorView(false)}
            viewPadding={viewPadding}
          />
        </Suspense>
      )}
      ...
      {/* Header del chat — fuera del container, position:fixed independiente del teclado iOS */}
