...
const STATUS: Record<TicketStatus, { color: string; bg: string; label: string; icon: string }> = {
  ...
  resolved:    { color: '#00c8a0', bg: 'rgba(0,200,160,0.1)',  label: 'Resuelto',     icon: '✅' },
  closed:      { color: '#64748b', bg: 'rgba(100,116,139,0.1)',label: 'Cerrado',      icon: '⚫' },
  escalated:   { color: '#a855f7', bg: 'rgba(168,85,247,0.1)', label: 'Escalado',     icon: '🆙' },
};
...
const CATEGORY_CFG: Record<TicketCategory, { label: string; icon: string; color: string }> = {
  ...
  chat:     { label: 'Chat / Mensajes',   icon: '💬', color: '#00c8a0' },
  app_bug:  { label: 'Error de la App',   icon: '🐛', color: '#f97316' },
  account:  { label: 'Cuenta / Perfil',   icon: '👤', color: '#3b82f6' },
...
function generateMock(): SupportData {
  ...
  return {
    ...
    byCategory: [
      ...
      { name: 'Chat / Mensajes',  count:  94, color: '#00c8a0', icon: '💬' },
      { name: 'Error App',        count:  87, color: '#f97316', icon: '🐛' },
      { name: 'Cuenta / Perfil',  count:  72, color: '#3b82f6', icon: '👤' },
    ...
    resolutionDist: [
      { range: '<15 min',   count: 38, color: '#00c8a0' },
      { range: '15-60 min', count: 52, color: '#3b82f6' },
...
export const SupportDashboard: React.FC = () => {
  ...
  // SLA color
  const slaColor = d.slaCompliance >= 90 ? '#00c8a0' : d.slaCompliance >= 75 ? '#f59e0b' : '#ef4444';
    ...
    <div style={{ color: '#f1f5f9' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '10px' }}>
        <div>
          ...
          <div style={{ fontSize: '11px', color: '#475569', marginTop: '3px' }}>FASE B — Operaciones · Tickets · Calidad · {d.lastUpdate}</div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '12px', marginBottom: '18px' }}>
        ...
        {/* Satisfaction score */}
        <div style={{ background: '#1e293b', borderRadius: '14px', padding: '18px', border: '1px solid #334155', textAlign: 'center' }}>
          <div style={{ fontSize: '10px', fontWeight: '700', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: '8px' }}>😊 Satisfacción</div>
          <div style={{ fontSize: '32px', fontWeight: '900', color: '#f59e0b', lineHeight: 1 }}>{d.satisfactionScore}</div>
          <div style={{ fontSize: '11px', color: '#475569', marginBottom: '8px' }}>/ 5.0</div>
          <Stars score={d.satisfactionScore} />
        </div>

        {/* SLA compliance gauge */}
        <div style={{ background: '#1e293b', borderRadius: '14px', padding: '18px', border: '1px solid #334155', textAlign: 'center' }}>
          <div style={{ fontSize: '10px', fontWeight: '700', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: '8px' }}>📋 Cumplimiento SLA</div>
          ...
          <div style={{ fontSize: '10px', color: '#475569' }}>objetivo: 90%</div>
        </div>

        {/* Open by priority */}
        <div style={{ background: '#1e293b', borderRadius: '14px', padding: '18px', border: '1px solid #334155' }}>
          <div style={{ fontSize: '10px', fontWeight: '700', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: '12px' }}>🎯 Abiertos por Prioridad</div>
            ...
            return (
              <div key={p} style={{ marginBottom: '7px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '3px' }}>
                  <span style={{ fontSize: '11px', color: '#94a3b8' }}>{cfg.icon} {cfg.label}</span>
        ...
        <div style={{ background: '#1e293b', borderRadius: '14px', padding: '18px', border: '1px solid #334155' }}>
          <div style={{ fontSize: '10px', fontWeight: '700', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: '12px' }}>⏱️ Tiempo de Resolución</div>
          {d.resolutionDist.map(r => {
            const total = d.resolutionDist.reduce((s, x) => s + x.count, 0);
            return (
              <div key={r.range} style={{ marginBottom: '7px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '3px' }}>
                  <span style={{ fontSize: '10px', color: '#94a3b8' }}>{r.range}</span>
        ...
        <div style={{ background: '#1e293b', borderRadius: '16px', padding: '18px', border: '1px solid #334155' }}>
          ...
          <ResponsiveContainer width="100%" height={180}>
            <AreaChart data={d.weeklyTrend}>
              ...
              <Line type="monotone" dataKey="resolved" name="Resueltos" stroke="#00c8a0" strokeWidth={2} dot={{ fill: '#00c8a0', r: 3 }} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px', marginBottom: '18px' }}>
            ...
            return (
              <div key={c.name} style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0', borderBottom: '1px solid #0f172a' }}>
                ...
                <div style={{ display: 'flex', gap: '6px' }}>
                  <span style={{ fontSize: '10px', color: '#475569' }}>{Math.round(c.count/total*100)}%</span>
        ...
        <div style={{ background: '#1e293b', borderRadius: '16px', padding: '18px', border: '1px solid #334155' }}>
          ...
          {d.topIssues.map((issue, i) => {
            ...
            const trendColor = issue.trend === 'up' ? '#ef4444' : issue.trend === 'down' ? '#00c8a0' : '#64748b';
            return (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '7px 0', borderBottom: '1px solid #0f172a' }}>
                <span style={{ fontSize: '11px', color: '#475569', width: '16px', flexShrink: 0 }}>{i + 1}.</span>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ height: '3px', background: '#0f172a', borderRadius: '2px', marginTop: '4px', overflow: 'hidden' }}>
        ...
        <div style={{ background: '#1e293b', borderRadius: '16px', padding: '18px', border: '1px solid #334155' }}>
          ...
          {d.byAgent.map(agent => (
            <div key={agent.name} style={{ padding: '8px', background: '#0f172a', borderRadius: '10px', marginBottom: '6px' }}>
              ...
              <div style={{ display: 'flex', gap: '10px', fontSize: '10px', color: '#64748b' }}>
                ...
                <span style={{ color: '#00c8a0' }}>Resueltos: <b>{agent.resolved}</b></span>
      ...
      <div style={{ background: '#1e293b', borderRadius: '16px', padding: '18px', border: '1px solid #334155' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px', flexWrap: 'wrap', gap: '10px' }}>
          ...
          <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
            <button onClick={() => { setStatus('all'); setPage(0); }} style={{ padding: '4px 12px', borderRadius: '8px', border: '1px solid #334155', background: statusFilter === 'all' ? '#334155' : 'transparent', color: statusFilter === 'all' ? '#f1f5f9' : '#64748b', fontSize: '11px', fontWeight: '700', cursor: 'pointer' }}>
              Todos
            </button>
              ...
              {paged.map(t => {
                ...
                return (
                  <tr key={t.id} style={{ borderBottom: '1px solid #0f172a' }}
                    ...
                    <td style={{ padding: '8px 10px', color: '#64748b', fontFamily: 'monospace', fontSize: '11px' }}>{t.id}</td>
                    ...
                    <td style={{ padding: '8px 10px', color: '#94a3b8', maxWidth: '180px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.subject}</td>
                    ...
                    <td style={{ padding: '8px 10px', color: '#64748b', whiteSpace: 'nowrap' }}>{t.agent || '—'}</td>
                    <td style={{ padding: '8px 10px', color: '#64748b', whiteSpace: 'nowrap' }}>{t.created}</td>
                    <td style={{ padding: '8px 10px', color: t.resolutionMin ? '#00c8a0' : '#475569', whiteSpace: 'nowrap', fontWeight: t.resolutionMin ? '700' : '400' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '12px', flexWrap: 'wrap', gap: '8px' }}>
          <span style={{ fontSize: '11px', color: '#475569' }}>{Math.min(page * PAGE_SIZE + 1, filtered.length)}–{Math.min((page + 1) * PAGE_SIZE, filtered.length)} de {filtered.length} tickets</span>
          <div style={{ display: 'flex', gap: '6px' }}>
            {[
      ...
      <div style={{ marginTop: '14px', padding: '10px 16px', background: '#1e293b', borderRadius: '10px', border: '1px solid #334155', display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '8px' }}>
        <span style={{ fontSize: '11px', color: '#475569' }}>🎧 Dashboard Soporte · Métricas de calidad · Actualización cada 30s</span>
        <div style={{ display: 'flex', gap: '12px' }}>
          <span style={{ fontSize: '11px', color: '#00c8a0' }}>SLA objetivo: 90%</span>
