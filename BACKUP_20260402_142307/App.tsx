...
import { RecargaMonederoModal, RetiroMonederoModal } from './WalletModals';
...
const App: React.FC = () => {
  ...
  const [userBalance, setUserBalance] = useState<number>(125450);
  ...
  const [aiMessages, setAiMessages] = useState<Array<{ id: string; type: 'user' | 'assistant'; content: string; timestamp: string }>>([
    {
      id: '1',
      type: 'assistant',
      content: '�Hola! Soy tu asistente EGCHAT. Puedo ayudarte con cualquier cosa: responder preguntas, enviar mensajes a contactos, gestionar tu dinero, y mucho mas. �En qu� puedo ayudarte?',
  ...
  // Funcion para procesar mensajes del asistente IA
  const processAiMessage = (userMessage: string) => {
    // Usar la versi?n avanzada
    ...
    const presentations: Record<string, Array<{ title: string; content: string; bullets: string[] }>> = {
      ...
      'tecnologia': [
        ...
        },
        {
          ...
          bullets: ['Beneficios clave', 'Llamada a la acci�n', 'Contacto']
        }
      ],
      'educacion': [
        {
  ...
  const logTransaction = (type: string, amount: number, status: string) => {
    ...
    console.log(' Transacci�n registrada en auditoraa:', newLog);
  };
      ...
      case 'apuestas':
        return (
          <svg style={iconStyle} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
            ...
            <circle cx="12" cy="14" r="2" stroke="none"/>
  ...
  const renderHeader = () => (
    <div style={{
      ...
      {/* Línea inferior ? fusi�n verde-blanco/negro-azul */}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
        ...
        <div style={{ position: 'relative' }}>
          ...
          <div className="hdr-tip" style={{ position: 'absolute', top: '110%', left: '50%', transform: 'translateX(-50%)', background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(8px)', border: '1px solid rgba(0,200,160,0.4)', borderRadius: '8px', padding: '5px 10px', whiteSpace: 'nowrap', fontSize: '10px', fontWeight: '600', color: '#00e5ff', opacity: 0, pointerEvents: 'none', transition: 'opacity 0.15s ease', zIndex: 1100 }}>Clima</div>
        </div>
        ...
        <div style={{ position: 'relative' }}>
          ...
          <div className="hdr-tip" style={{ position: 'absolute', top: '110%', right: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(8px)', border: '1px solid rgba(0,200,160,0.4)', borderRadius: '8px', padding: '5px 10px', whiteSpace: 'nowrap', fontSize: '10px', fontWeight: '600', color: '#00e5ff', opacity: 0, pointerEvents: 'none', transition: 'opacity 0.15s ease', zIndex: 1100 }}>Notificaciones</div>
        </div>
        ...
        <div style={{ position: 'relative' }}>
          ...
          <div className="hdr-tip" style={{ position: 'absolute', top: '110%', right: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(8px)', border: '1px solid rgba(0,180,230,0.4)', borderRadius: '8px', padding: '5px 10px', whiteSpace: 'nowrap', fontSize: '10px', fontWeight: '600', color: '#1e90ff', opacity: 0, pointerEvents: 'none', transition: 'opacity 0.15s ease', zIndex: 1100 }}>Mena principal</div>
        </div>
      ...
      return (
        <div onClick={() => { setSelectedNotif(null); setShowNotifications(false); }}
          ...
          <div onClick={e => e.stopPropagation()} style={{
            ...
            borderRadius: '14px', border: `1px solid ${selectedNotif.color}30`,
            boxShadow: `0 8px 32px rgba(0,0,0,0.7), 0 0 0 1px ${selectedNotif.color}15`,
            overflow: 'hidden'
          }}>
            {/* Header detalle */}
  ...
  const renderWeatherModal = () => {
    ...
    return (
      <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 1100, display: 'flex', alignItems: 'flex-start', justifyContent: 'flex-end', padding: '56px 12px 0' }}
        ...
        <div style={{ background: '#FFFFFF', backdropFilter: 'blur(20px)', borderRadius: '14px', border: '1px solid rgba(0,0,0,0.08)', width: '260px', maxHeight: 'calc(100vh - 80px)', overflowY: 'auto', padding: '16px', boxShadow: '0 8px 32px rgba(0,0,0,0.1)' }}
  ...
  const renderTimeModal = () => {
    ...
    return (
      <div style={{ position: 'fixed', inset: 0, zIndex: 1200, background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
        ...
        <div onClick={e => e.stopPropagation()} style={{ background: '#FFFFFF', borderRadius: '16px', padding: '20px', width: '260px', boxShadow: '0 8px 32px rgba(0,0,0,0.15)' }}>
  ...
  const renderMenuPanel = () => {
    ...
    return (
      <div style={{
        ...
        {[
          { id: 'perfil', label: 'Mi Perfil', icon: 'contactos' },
          { id: 'nuevo-contacto', label: 'Nuevo contacto', icon: 'contactos' },
          { id: 'crear-grupo', label: 'Crear grupo', icon: 'mensajes' },
          { id: 'ayuda', label: 'Ayuda', icon: 'notification' },
          { id: 'salir', label: 'Cerrar Sesi?n', icon: 'close' }
        ].map((item, index) => (
          <button
            key={item.id}
            onClick={() => {
            ...
            style={{
              ...
              color: item.id === 'salir' ? '#ef4444' : '#0d0d0d',
              cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '12px',
              fontSize: '13px', fontWeight: '500', outline: 'none', textAlign: 'left'
            }}
          >
            <span style={{ color: item.id === 'salir' ? '#ef4444' : '#00c8a0', display: 'flex' }}>
  ...
  const renderWallpaperCatalog = () => {
    ...
    return (
      <div style={{ position: 'fixed', inset: 0, zIndex: 2000, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'flex-end' }}
        ...
        <div onClick={e => e.stopPropagation()} style={{ width: '100%', background: '#FFFFFF', borderRadius: '20px 20px 0 0', padding: '14px 14px 24px', maxHeight: '80vh', overflowY: 'auto' }}>
                ...
                <div key={cw.id} style={{ position: 'relative' }}>
                  <button onClick={() => { setSelectedWallpaper(cw.id); setShowWallpaperCatalog(false); }}
                    ...
                    <div style={{ height: '130px', position: 'relative', overflow: 'hidden', background: '#111' }}>
                      ...
                      {cw.type === 'video' && (
                        <div style={{ position: 'absolute', top: '2px', left: '2px', background: 'rgba(0,0,0,0.7)', borderRadius: '3px', padding: '1px 3px', fontSize: '6px', fontWeight: '700', color: 'white' }}></div>
                      )}
                    </div>
  ...
  const renderLayoutPanel = () => {
    ...
    return (
      ...
      >
        <div onClick={e => e.stopPropagation()} style={{ width: '100%', background: '#FFFFFF', borderRadius: '20px 20px 0 0', padding: '16px 16px 32px', maxHeight: '75vh', overflowY: 'auto' }}>
          {/* Handle */}
  ...
  const renderRadialMenu = () => {
    ...
    const menuItems = [
      ...
      { id: 'edu',        label: 'EDUCACI�N',  angle: 120 },
  ...
  const renderBottomNavigation = () => {
    const allViews = ['home', 'mensajeria', 'monedero', 'servicios', 'ajustes', 'ai-assistant', 'estados', 'apuestas', 'cemac', 'mitaxi'];
  ...
  const renderHomeView = () => {
    ...
    // Layout: Compacto - accesos r�pidos en grid
    ...
    <div style={{
      ...
      <div style={{ marginTop: '12px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '8px' }}>
          {[
            { id: 'estados',  label: 'Estados',         grad: 'linear-gradient(145deg,#7c3aed,#c026d3)', icon: renderIcon('estados',  24) },
            { id: 'apuestas', label: 'Juegos y Apuestas', grad: 'linear-gradient(145deg,#b45309,#dc2626)', icon: renderIcon('apuestas', 24) },
            { id: 'cemac',    label: 'CEMAC',            grad: 'linear-gradient(145deg,#065f46,#0369a1)', icon: renderIcon('cemac',    24) },
            { id: 'mitaxi',   label: 'MiTaxi',           grad: 'linear-gradient(145deg,#92400e,#d97706)', icon: renderIcon('mitaxi',   24) },
          ].map(item => (
            <button
              key={item.id}
            ...
            >
              ...
              <span style={{ fontSize: '10px', color: '#000', fontWeight: '600', textAlign: 'center', lineHeight: '1.2', maxWidth: '60px' }}>{item.label}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding: '56px 0 0', height: '100vh', display: 'flex', flexDirection: 'column', background: '#F7F8FA', overflow: 'hidden' }}>
        ...
        <div className="scroll-container" style={{ flex: 1, overflowY: 'scroll', paddingBottom: '90px' }}>
          ...
          <Section title="Herramientas">
            ...
            <Btn label="Asistente IA" icon="ai" color="#1485EE" onClick={() => setCurrentView('ai-assistant')} />
          ...
          return (
            <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', paddingTop: '56px', overflow: 'hidden' }} onClick={() => { if(showChatMenu) setShowChatMenu(false); }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 10px', background: '#ffffff', borderBottom: '1px solid rgba(0,0,0,0.08)', flexShrink: 0 }}>
                ...
                <div style={{ width: '34px', height: '34px', borderRadius: '50%', background: `${color}25`, border: `2px solid ${color}60`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: '700', color, flexShrink: 0 }}>
                  {initials}
                </div>
                <div style={{ flex: 1 }}>
              ...
              {showChatMenu && (
                <div style={{position:'fixed',inset:0,zIndex:200}} onClick={()=>setShowChatMenu(false)}>
                  <div style={{position:'absolute',top:'100px',right:'8px',background:'#fff',borderRadius:'14px',boxShadow:'0 4px 24px rgba(0,0,0,0.15)',width:'240px',overflow:'hidden',border:'1px solid #F0F2F5'}}
                    onClick={e=>e.stopPropagation()}>
                    {/* Sección principal */}
                    {[
                      ...
                      <button key={i} onClick={item.action} style={{width:'100%',background:'none',border:'none',padding:'10px 14px',display:'flex',alignItems:'center',gap:'10px',cursor:'pointer',outline:'none',borderBottom:'1px solid #F9FAFB',textAlign:'left'}}
                    ...
                    {[
                      ...
                      <button key={i} onClick={item.action} style={{width:'100%',background:'none',border:'none',padding:'10px 14px',display:'flex',alignItems:'center',gap:'10px',cursor:'pointer',outline:'none',borderBottom:i<arr.length-1?'1px solid #F9FAFB':'none',textAlign:'left'}}
                    ...
                    {[
                      ...
                      >
                        <div style={{
                          ...
                          height: '56px',
                          borderRadius: '50%',
                          background: `linear-gradient(135deg, ${contact.color}40 0%, ${contact.color}20 100%)`,
                          border: `2.5px solid ${contact.color}`,
                          display: 'flex',
                          alignItems: 'center',
                          justifyContent: 'center',
                          color: contact.color,
                          flexShrink: 0,
                          boxShadow: `0 0 16px ${contact.color}50, inset 0 0 8px ${contact.color}20`,
                          transition: 'all 0.2s ease',
                          fontSize: '18px',
                          fontWeight: '700',
                          letterSpacing: '0.5px'
                    ...
                    {[
                      ...
                      >
                        <div style={{
                          ...
                          height: '56px',
                          borderRadius: '50%',
                          background: `linear-gradient(135deg, ${group.color}40 0%, ${group.color}20 100%)`,
                          border: `2.5px solid ${group.color}`,
                          display: 'flex',
                          alignItems: 'center',
                          justifyContent: 'center',
                          color: group.color,
                          flexShrink: 0,
                          boxShadow: `0 0 16px ${group.color}50, inset 0 0 8px ${group.color}20`,
                          transition: 'all 0.2s ease',
                          fontSize: '18px',
                          fontWeight: '700',
                          letterSpacing: '0.5px'
          ...
          <div style={{
            ...
            <div style={{
              ...
              {['all', 'sent', 'received', 'payment', 'deposit', 'withdrawal'].map((filter) => (
                ...
                >
                  {filter === 'all' ? 'Todas' : filter === 'sent' ? 'Enviadas' : filter === 'received' ? 'Recibidas' : filter === 'payment' ? 'Pagos' : filter === 'deposit' ? 'Dep�sitos' : 'Retiros'}
                </button>
              ))}
            </div>
              ...
              {aiMessages.length === 0 ? (
                ...
                aiMessages.map((msg) => (
                  ...
                  >
                    ...
                    >
                      <div style={{ marginBottom: '4px', whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
                        {msg.content}
                      </div>
                      <div style={{ fontSize: '9px', color: '#6b7280' }}>
                        {msg.timestamp}
  ...
  return (
    <div style={{
      ...
      {currentView === 'home' && (
        <button 
          ...
          style={{
            ...
            justifyContent: 'center',
            zIndex: 1001,
            boxShadow: '0 3px 12px rgba(139, 92, 246, 0.5), 0 0 20px rgba(124, 58, 237, 0.3)',
            outline: 'none',
            touchAction: 'none',
            userSelect: 'none',
            padding: 0,
      ...
      {showQRModal && (() => {
        ...
        return (
          <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000, backdropFilter: 'blur(6px)' }}
            ...
            <div style={{ background: '#FFFFFF', width: '85%', maxWidth: '320px', borderRadius: '24px', overflow: 'hidden', boxShadow: '0 20px 60px rgba(0,0,0,0.25)' }}
              onClick={(e) => e.stopPropagation()}>
      ...
      {showSalaryReloadModal && <RecargaMonederoModal onClose={() => setShowSalaryReloadModal(false)} userBalance={userBalance} onCredit={(n) => setUserBalance(p => p + n)} />}

      {/* ── MODAL RETIRO MONEDERO ─────────────────────────────────────────── */}
      {showCardWithdrawalModal && <RetiroMonederoModal onClose={() => setShowCardWithdrawalModal(false)} userBalance={userBalance} onDebit={(n) => setUserBalance(p => p - n)} />}
