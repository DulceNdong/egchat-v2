...
const BASE = (import.meta as any).env?.VITE_API_URL || 'https://egchat-backend.onrender.com';
...
export const chatAPI = {
  ...
  getMessages:  (chatId:string, page=1) => get<any[]>(`/chats/${chatId}/messages?page=${page}`),
  sendMessage:  (chatId:string, text:string, type='text') => post<any>(`/chats/${chatId}/messages`, { text, type }),
  sendFile:     async (chatId:string, file:File) => {
    const fd = new FormData(); fd.append('file', file);
    const res = await fetch(`${BASE}/chats/${chatId}/files`, { method:'POST', body:fd });
    return res.json();
  },
  createGroup:  (name:string, members:string[]) => post<any>('/api/chats/groups', { name, members }),
