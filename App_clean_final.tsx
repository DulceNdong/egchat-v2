...
const App: React.FC = () => {
  ...
  const [allContacts, setAllContacts] = useState<Array<{ id: string; name: string; phone: string; avatar: string; status: 'online' | 'offline' | 'away'; addedDate: string }>>([
    { id: '1', name: 'Juan P?rez', phone: '+240 222 111111', avatar: 'JP', status: 'online', addedDate: '15/03/2026' },
    ...
    { id: '4', name: 'Ana Mart?nez', phone: '+240 222 444444', avatar: 'AM', status: 'offline', addedDate: '12/03/2026' },
    { id: '5', name: 'Pedro Sanchez', phone: '+240 222 555555', avatar: 'PS', status: 'online', addedDate: '11/03/2026' },
    { id: '6', name: 'Sof?a Rodr?guez', phone: '+240 222 666666', avatar: 'SR', status: 'offline', addedDate: '10/03/2026' }
  ]);
  ...
  const [allGroups, setAllGroups] = useState<Array<{ id: string; name: string; description: string; members: number; avatar: string; createdDate: string; lastMessage: string; unread: number }>>([
    ...
    { id: '4', name: 'Proyecto', description: 'Proyecto especial',      members: 6,  avatar: 'project',  createdDate: '01/03/2026', lastMessage: 'Deadline pr?xima semana',    unread: 1 }
  ]);
  ...
  const renderNotificationsPanel = () => {
    ...
    const notifs = [
      ...
      {
        id: 2, icon: 'message-square', label: 'Mensaje nuevo', desc: 'Mar?a: ?C?mo est?s?', time: '12m', color: '#00b4e6',
        detail: {
          title: 'Mensaje nuevo',
          from: 'Maria Garcia',
          fromPhone: '+240 222 222222',
          fromAvatar: 'MG',
          message: '?C?mo est?s? Hace tiempo que no hablamos, espero que todo vaya bien ',
          date: '19/03/2026 a 10:17',
          status: 'No le?do',
          statusColor: '#00b4e6',
          action: () => { setSelectedChat({ id: 2, type: 'individual', title: 'Maria Garcia', subtitle: '?C?mo est?s? Hace tiempo que no hablamos...', time: '10:17', status: 'online', icon: 'contactos', color: '#00c8a0', initials: 'MG' }); setCurrentView('mensajeria'); },
      ...
      <div style={{ position:'fixed', top:'56px', right:'8px', width:'230px', background:'rgba(255,255,255,0.35)', backdropFilter:'blur(28px) saturate(200%)', WebkitBackdropFilter:'blur(28px) saturate(200%)', borderRadius:'16px', border:'1.5px solid rgba(255,255,255,0.6)', boxShadow:'0 8px 32px rgba(0,0,0,0.15)', zIndex:1001, overflow:'hidden' }}>
        ...
        {menuItems.map((item, i) => (
          <button key={item.id} onClick={() => {
            ...
            else if (item.id==='salir') { if(window.confirm('?Cerrar sesi?n?')) alert('Sesi?n cerrada'); }
            setShowMenu(false);
          }}
    ...
    return (
      <div style={{
        ...
        {navItems.map((item) => (
          ...
          >
            {/* Indicador activo ? l?nea blanca arriba */}
            {currentView === item.id && (
      ...
      case 'mensajeria':
        ...
        if (selectedChat) {
          ...
          const msgs = chatMessages[chatId] || [
            { id: '1', from: 'them' as const, text: sc.subtitle || '?C?mo est?s?', time: sc.time || '10:17', status: 'read' as const },
            { id: '2', from: 'me' as const, text: 'Hola! Todo bien por aqu?', time: '10:18', status: 'read' as const },
          ...
          const sendChatMessage = () => {
            ...
            // pending ? delivered despu?s de 1s, ? read despu?s de 3s
            ...
            <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', paddingTop: '56px', overflow: 'hidden' }} onClick={() => { if(showChatMenu) setShowChatMenu(false); }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 10px', background: '#ffffff', borderBottom: '1px solid rgba(0,0,0,0.08)', flexShrink: 0 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: '10px', color: sc.status === 'online' ? '#00c8a0' : sc.status === 'away' ? '#f59e0b' : '#9ca3af' }}>{sc.status === 'online' ? 'En l?nea' : sc.status === 'away' ? 'Ausente' : 'Desconectado'}</div>
                </div>
                      ...
                      <div style={{ fontSize: '9px', color: '#9ca3af', marginTop: '3px', textAlign: 'right', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '3px' }}>
                        ...
                        {msg.from === 'me' && showReadReceipts && (
                          <span style={{ display: 'flex', gap: '3px', alignItems: 'center', marginLeft: '2px' }}>
                            {/* 3 puntos: amarillo=pendiente, verde=entregado, azul=le?do */}
                ...
                <div style={{ background: '#FFFFFF', borderTop: '1px solid rgba(0,0,0,0.07)', padding: '14px 16px', flexShrink: 0 }}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px' }}>
                    {[
                        ...
                        action: () => {
                          ...
                          if(navigator.geolocation){
                            navigator.geolocation.getCurrentPosition(pos=>{
                              ...
                              setChatMessages(prev=>({...prev,[chatId]:[...(prev[chatId]||[]),{id:Date.now().toString(),from:'me',text:`?? Mi ubicaci?n\nhttps://maps.google.com/?q=${lat},${lng}`,time,status:'pending'}]}));
                            }, ()=>{
                              const now=new Date(); const time=`${now.getHours().toString().padStart(2,'0')}:${now.getMinutes().toString().padStart(2,'0')}`;
                              const chatId=sc.id?.toString()||'';
                      ...
                      {
                        label: 'M?s', color: '#6b7280', bg: '#F3F4F6',
                ...
                {expandFavoriteContacts && (
                  <div style={{
                    ...
                    {[
                      ...
                      { id: 2, name: 'Mar?a', initials: 'MA', color: '#00c8a0' },
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
                      <div style={{ 
                        ...
                        {chat.status === 'online' ? 'En l?nea' : chat.status === 'away' ? 'Ausente' : 'Desconectado'}
                      </div>
                    </div>
                  </div>
                ))}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  ...
                  <div style={{
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      ...
                      <div>
                        <div style={{ fontSize: '10px', color: '#6b7280', marginBottom: '2px' }}>Tel?fono</div>
                        <div style={{ fontSize: '12px', color: '#0d0d0d', fontWeight: '500' }}>{userProfile.phone}</div>
                      </div>
                      <div>
                        <div style={{ fontSize: '10px', color: '#6b7280', marginBottom: '2px' }}>Pa?s</div>
