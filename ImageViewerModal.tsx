...
export default function ImageViewerModal({ url, onClose, chats, onForward, onShowToast }: Props) {
  ...
  const [showControls, setShowControls] = useState(false); // controles ocultos por defecto
  ...
  // Doble tap para zoom, tap simple para mostrar/ocultar controles
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 5000, background: '#000', display: 'flex', flexDirection: 'column', animation: 'imgViewerIn 0.18s ease' }}>
      ...
      {/* ── TOP BAR — solo visible al tocar ── */}
      {(showControls || showEdit) && (
      <div style={{
        ...
        animation: 'imgViewerIn 0.15s ease',
      }}>
        {/* Cerrar */}
      ...
      {/* ── BOTTOM BAR — solo Guardar y Reenviar, visible al tocar ── */}
      {showControls && !showEdit && (
        <div style={{
          ...
          <button className="iv-btn" onClick={() => setShowForward(true)} style={{ ...btnStyle(), background: 'rgba(0,200,160,0.2)', border: '1px solid rgba(0,200,160,0.4)' }}>
            ...
            <span style={{ fontSize: '11px', color: '#00c8a0' }}>Enviar</span>
          </button>
        </div>
      )}
