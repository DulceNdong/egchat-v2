...
export default function HomeScreen() {
  ...
  const openFab = () => {
    ...
    Animated.parallel([
      ...
      Animated.timing(fabOverlayOpacity, { toValue: 1, duration: 250, useNativeDriver: true }),
  ...
  return (
    <SafeAreaView style={[st.container, { backgroundColor: C.bgPrimary }]} edges={['bottom', 'left', 'right']}>
      ...
      {/* ════════════════════════════════════════════════════════
          FAB RADIAL — 1 solo círculo alrededor del botón
      ════════════════════════════════════════════════════════ */}
      {fabOpen && (() => {
        const { width: SW } = Dimensions.get('window');
        const FAB_CX     = SW / 2;
        const FAB_BOTTOM = 210 + 30;  // centro vertical del FAB
        const COUNT      = FAB_SERVICES.length; // 13 ítems
        const R          = 150;       // un solo anillo
        const ITEM_SIZE  = 52;
        ...
        return FAB_SERVICES.map((svc, i) => {
          ...
          const finalY = FAB_BOTTOM - Math.sin(angleRad) * R - ITEM_HALF;

          const animScale = fabItemAnims[i].interpolate({
            inputRange:  [0, 0.6, 1],
          ...
          return (
            ...
            >
              <TouchableOpacity
                style={[st.fabRadialBtn, { borderColor: svc.color }]}
                onPress={() => navigateFab(svc.route)}
                activeOpacity={0.75}
              >
                <View style={[st.fabRadialBg, { backgroundColor: svc.color + '28' }]} />
...
const st = StyleSheet.create({
  ...
  fabRadialBtn: {
    ...
    justifyContent: 'center',
    borderWidth: 1.8,
    borderColor: 'rgba(0,200,160,0.9)',
    overflow: 'hidden',
    // sombra suave
    shadowColor: '#000',
