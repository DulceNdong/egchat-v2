// Módulo Mis Facturas — paridad ServiciosModules.tsx web
import React, { useState, useEffect } from 'react';
import { View, Text, TouchableOpacity, StyleSheet } from 'react-native';
import { INITIAL_BILLS, BILL_CATEGORIES, Bill } from '../../data/serviciosFinancieros';
...
const FILTER_OPTS = [
  ...
  { id: 'vencida', label: 'Vencida' }, { id: 'pagada', label: 'Pagada' },
];
...
export const FacturasModal: React.FC<Props> = ({ visible, onClose, userBalance = 100000 }) => {
  ...
  const [form, setForm] = useState({ service: '', provider: '', amount: '', dueDate: '', ref: '', catId: '' });
  const [payMethod, setPayMethod] = useState('');
  const [balance, setBalance] = useState(userBalance);
  const [refreshKey, setRefreshKey] = useState(0);

  useEffect(() => { if (visible) setBalance(userBalance); }, [visible, userBalance]);
  useEffect(() => { if (!visible) { setScreen('home'); setSelected(null); } }, [visible]);
  ...
  const addBill = () => {
    ...
    setBills(p => [...p, {
      ...
      amount: parseInt(form.amount, 10), dueDate: form.dueDate, status: 'pendiente',
  ...
  const fixedTop = screen === 'home' ? (
    <View style={{ paddingHorizontal: 16, paddingBottom: 8 }}>
      ...
      />
      <FilterChips options={FILTER_OPTS} value={filter} onChange={id => setFilter(id as typeof filter)} activeColor="#C47D2A" />
    </View>
  ) : undefined;

  const titles: Record<FScreen, string> = {
    home: 'Mis Facturas', add: 'Nueva Factura', detail: selected?.service || 'Factura',
    pay: 'Pagar Factura', success: 'Pago completado',
  };
    ...
    >
            ...
            <TouchableOpacity key={b.id} style={s.billRow} onPress={() => { setSelected(b); setScreen('detail'); }}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <View style={s.billBottom}>
                  <Text style={s.billMeta}>{b.provider} — Vence {b.dueDate}</Text>
      ...
      {screen === 'add' && (
        <View>
          ...
          <View style={s.catGrid}>
            {BILL_CATEGORIES.map(c => (
              ...
              >
                ...
                <Text style={[s.catLabel, form.catId === c.id && { color: c.color }]}>{c.label}</Text>
              </TouchableOpacity>
            ))}
          </View>
          ...
          <FormField placeholder="Fecha de vencimiento (DD/MM/AAAA)" value={form.dueDate} onChangeText={v => setF('dueDate', v)} />
          <PrimaryButton label="Guardar factura" color="#C47D2A" disabled={!form.service || !form.amount || !form.dueDate} onPress={addBill} />
        </View>
      )}
      ...
      {screen === 'detail' && selected && (
        <View>
          <View style={s.detailCard}>
            ...
            {[['Referencia', selected.ref], ['Vencimiento', selected.dueDate], ['Estado', selected.status], ['Importe', `${selected.amount.toLocaleString()} XAF`]].map(([l, v]) => (
          ...
          {selected.status !== 'pagada' ? (
            ...
            <View style={s.paidBanner}><Text style={s.paidText}>✅ Esta factura ya está pagada</Text></View>
          )}
        </View>
      )}
      ...
      {screen === 'pay' && selected && (
        <View>
          ...
          {([['monedero', 'Monedero EGCHAT'], ['banco', 'Cuenta bancaria'], ['tarjeta', 'Tarjeta de débito']] as const).map(([id, label]) => (
          ...
          <PrimaryButton
            ...
            color="#C47D2A"
            disabled={!payMethod || selected.amount > balance}
            onPress={payBill}
          />
        </View>
      )}
      ...
      {screen === 'success' && selected && (
        <View style={s.success}>
          ...
          <PrimaryButton label="Listo" onPress={() => { setSelected(null); setScreen('home'); }} color="#C47D2A" />
        </View>
      )}
    </FinancialModuleShell>
  );
