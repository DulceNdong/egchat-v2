...
export function QuickTransferModal({
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (limitCheck.requiresPin) {
        setError('Transferencia mayor al 50% del límite diario. Activa PIN en Seguridad.');
        return;
      }
      const to = recipientPhone || recipientId || contactName;
      await walletAPI.transfer(to, num, `Chat: ${contactName}`);
      await updateLimitForTransaction('transfer', num);
